...
from methods.zeidel_matrix import clean_solve_zeidel_matrix
...
def test_identity_matrix():
    ...
    result = clean_solve_zeidel_matrix(1e-10, A, b)
...
def test_random_diagonally_dominant_systems(n):
    ...
    result = clean_solve_zeidel_matrix(1e-10, A, b, maxiter=10000)
...
def test_input_arrays_are_not_modified():
    ...
    clean_solve_zeidel_matrix(1e-10, A, b)
...
def test_low_maxiter_returns_none_or_bad_accuracy():
    A = np.array(
        [
            [10.0, 1.0],
            [1.0, 10.0],
    ...
    result = clean_solve_zeidel_matrix(1e-12, A, b, maxiter=1)
    ...
    if result is None:
        ...
        assert not np.allclose(x, expected_x, atol=1e-12) or not np.allclose(
            residual, 0, atol=1e-12
        )
